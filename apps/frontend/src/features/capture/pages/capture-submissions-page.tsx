          ...
          <CardContent className="px-6 py-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="space-y-1">
                ...
                <p className="text-sm text-foreground/80">Capture submission {deletedJobId} was removed.</p>
              </div>
              <NavLink to="/capture">
                <Button variant="outline">Create another capture</Button>
              </NavLink>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="bg-white/92">
        <CardHeader className="flex flex-wrap items-start justify-between gap-6 md:flex-row">
          <div className="space-y-3">
            <Badge variant="outline">Your submissions</Badge>
            <CardTitle className="text-2xl">Review saved capture jobs</CardTitle>
            <CardDescription className="max-w-2xl">
              Each submission is a persisted capture package. This list view is intentionally card-based so the same
              card pattern can be reused later for admin review screens.
            </CardDescription>
          </div>
          <div className="flex gap-3">
            <NavLink to="/capture">
              <Button variant="outline">Back to capture</Button>
            </NavLink>
      ...
      {jobs.length === 0 ? (
        <Card className="bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(244,242,236,0.95))]">
          <CardContent className="flex flex-col items-start gap-4 px-6 py-8">
            <Badge variant="warn">No submissions yet</Badge>
            <div>
              <h3 className="text-xl font-semibold tracking-[-0.03em]">Create the first capture package</h3>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Once the review step is submitted, the backend stores a capture job and it will appear here.
              </p>
            </div>
            <NavLink to="/capture/interview">
